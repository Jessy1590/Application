...
import { ArrowLeft, ShieldAlert, Edit2, Save, X, Filter } from 'lucide-react';
import { fetchQualityEvents, updateQualityEvent } from '../services/qualityService';
...
const STATUS_OPTIONS = ['ouvert', 'en_analyse', 'cloture'];
          ...
          <tbody className="divide-y divide-slate-100">
            {filtered.map(ev => (
              <tr key={ev.id} className="hover:bg-slate-50">
                ...
                <td className="p-4 align-top">
                  <span className="text-xs bg-slate-100 px-2 py-1 rounded">{ev.status}</span>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div>
                ...
                <textarea rows={3} value={editing.capa_action || ''} onChange={e => setEditing({ ...editing, capa_action: e.target.value })}
                  className="w-full p-2 border rounded-lg" placeholder="Mesures à mettre en place..." />
              </div>
              <div>
