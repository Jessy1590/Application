/**
 * Impression Location — fiche A4 (iframe same-document).
 * Branding : PharmaOS (pas Pharmacie Evreux).
 */
import * as LocationRules from './locationRules.js';
import * as LocationData from './locationService.js';


  const R = () => LocationRules;
  ...
  function motifContactLabel(motif) {
    ...
    const tpl = LocationRules?.templateMotifFor?.(motif);
  ...
  function buildSuiviEventRows(dossier) {
    const rows = [];
    const LC = typeof window !== 'undefined' ? window.LocationContact : null;
    const resultatLabels = LC?.APPEL_RESULTAT_LABELS || {};
    const statutLabels = {
      ...CONTACT_STATUT_LABELS,
      ...(LC?.APPEL_STATUT_LABELS || {}),
    };
  ...
  async function loadAttentionLines(dossier) {
    ...
    if (!type || !LocationData?.listChampsCreation) return [];
  ...
  function customCreationLines(params, etapeId, extra) {
    ...
    if (!LocationData?.listCreationFieldsForEtape) return [];
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  <p class="print-meta">PharmaOS — Location — version ${esc(versionCourteFr())}</p>
  ...
  async function printFiche(dossier) {
    ...
    try {
      if (LocationData?.loadParams) params = (await LocationData.loadParams()) || {};
    } catch (_) {
      params = {};
    }
    try {
      if (LocationData?.listChampsCreation) {
...
<style>
  ...
  <h1>Tableau des locations — PharmaOS</h1>
  ...
  function printContactList(items) {
    const LC = typeof window !== 'undefined' ? window.LocationContact : null;
    const RESULTAT_LABELS = LC?.APPEL_RESULTAT_LABELS || {};
    const STATUT_LABELS = LC?.APPEL_STATUT_LABELS || {};
...
<style>
  ...
  <h1>PharmaOS — Locations (Contact)</h1>
...
export { printFiche, printTableau, printContactList, buildFicheHtml, buildSuiviEventRows };
