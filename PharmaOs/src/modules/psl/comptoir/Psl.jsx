...
/**
 * @param {{ view?: 'reception'|'delivrance'|null }} props
 */
export default function Psl({ view: viewProp = null } = {}) {
  const { user } = useAuth();
  const mode = viewProp === 'reception' ? 'reception' : 'delivrance';
    ...
    <div className="w-full h-full flex flex-col bg-slate-50 text-slate-800">
      <div className="flex-1 overflow-y-auto p-6 max-w-2xl">
        <h2 className="text-xl font-bold mb-1 flex items-center gap-2">
          ...
          {mode === 'reception' ? 'MDS — Réception' : 'MDS — Délivrance'}
        </h2>
