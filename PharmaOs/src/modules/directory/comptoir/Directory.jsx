...
export default function Directory() {
  ...
  // Filtrage des contacts (par type + sous-type partenaire + recherche texte)
  const filteredContacts = contacts.filter((c) => {
    const matchType = c.type === activeTab;
    const matchPartenaire = activeTab !== 'commercial_partner'
      || partenaireFilter === 'all'
      || c.partenaire_type === partenaireFilter;
    const typeLabel = labelPartenaireType(c.partenaire_type, c.partenaire_type_autre) || '';
    const searchString = `${c.nom || ''} ${c.prenom || ''} ${c.specialite || ''} ${c.infos_contact || ''} ${typeLabel}`.toLowerCase();
    const matchSearch = searchString.includes(searchQuery.toLowerCase());
    return matchType && matchPartenaire && matchSearch;
  });
      ...
      <div className="bg-white px-6 pt-6 pb-4 border-b border-slate-200 shadow-sm shrink-0">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex bg-slate-100 p-1 rounded-lg">
            <button
              onClick={() => { setActiveTab('health_professional'); setPartenaireFilter('all'); setExpandedId(null); }}
            ...
            <button
              onClick={() => { setActiveTab('commercial_partner'); setPartenaireFilter('all'); setExpandedId(null); }}
              className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
          ...
          {activeTab === 'commercial_partner' && (
            <div className="flex flex-wrap gap-1.5">
              <button
                type="button"
                onClick={() => { setPartenaireFilter('all'); setExpandedId(null); }}
                className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${
                  partenaireFilter === 'all' ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
                }`}
              >
                Tous types
              </button>
              {PARTENAIRE_TYPES.map((t) => (
                <button
                  key={t.value}
                  type="button"
                  onClick={() => { setPartenaireFilter(t.value); setExpandedId(null); }}
                  className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${
                    partenaireFilter === t.value ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-200 text-slate-600 hover:bg-slate-50'
                  }`}
                >
                  {t.label}
                </button>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {filteredContacts.map(contact => {
              ...
              const partnerLabel = labelPartenaireType(contact.partenaire_type, contact.partenaire_type_autre);
              const subtitle = activeTab === 'health_professional'
                ? contact.specialite
                : (partnerLabel || contact.infos_contact);
                          ...
                          <>
                            {partnerLabel && (
                              <div className="flex items-start gap-2">
                                ...
                                <div>
                                  <p className="text-slate-500 text-xs">Type de partenaire</p>
                                  <p className="font-medium text-slate-800">{partnerLabel}</p>
                                </div>
                              </div>
                            )}
